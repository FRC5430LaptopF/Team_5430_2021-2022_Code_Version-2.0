  ...
  private static boolean test = true;
    public void run(){
       ...
       if(Robot.joystickButton(1) && count == 0 || deltaV != VariableSpeed.getMultiplier()){
           ...
           //speed changed {to 0.7 4/1/22 (previously .9)
if(!(leftMotor.get() == 0) |!(Robot.controller0).getRawButtonReleased(1) ){
    deltaV = VariableSpeed.getMultiplier();
           leftMotor.set(ControlMode.PercentOutput, VariableSpeed.getMultiplier() );// originally 0.7 for both, changed at Cap. Cit. Clas. to Ramone's request
           rightMotor.set(ControlMode.PercentOutput, VariableSpeed.getMultiplier() );
           count = 1;
}
       }
...
roughVelo =0;
        ...
        configs.voltageCompSaturation = 11.0;
switch(port){
    case 0:
